...
def _get_page_sources(nodes: list[dict], page_id: str) -> list[str]:
    """Extract frontmatter 'sources' field from a node's markdown content."""
    for n in nodes:
        if n["id"] == page_id:
            match = re.search(r'^sources:\s*\[([^\]]*)\]', n.get("markdown", ""), re.MULTILINE)
            if match:
...
def _source_overlap(nodes: list[dict], src: str, tgt: str) -> float:
    """Signal 2: Jaccard similarity of frontmatter sources × 4.0."""
    s1 = _get_page_sources(nodes, src)
    s2 = _get_page_sources(nodes, tgt)
    if not s1 or not s2:
        return 0.0
    ...
    if intersection == 0:
        return 0.0
    return (intersection / len(set1 | set2)) * 4.0


def _adamic_adar(neighbor_index: dict[str, set[str]], src: str, tgt: str) -> float:
    """Signal 3: Adamic-Adar score (weighted by log-degree)."""
    common = neighbor_index.get(src, set()) & neighbor_index.get(tgt, set())
    if not common:
        return 0.0
    score = sum(1.0 / max(len(neighbor_index.get(n, set())) or 1, 1) for n in common)
    return score * 1.5


def _type_affinity(nodes: list[dict], src: str, tgt: str) -> float:
    """Signal 4: Predefined type affinity score."""
    node_types = {n["id"]: n.get("type", "unknown") for n in nodes}
    t1 = node_types.get(src, "unknown")
    t2 = node_types.get(tgt, "unknown")
    return TYPE_AFFINITY.get((t1, t2), TYPE_AFFINITY.get((t2, t1), 0.3))


def _build_neighbor_index(edges: list[dict]) -> dict[str, set[str]]:
...
def calculate_edge_weights(nodes: list[dict], edges: list[dict]) -> list[dict]:
    """Compute five-signal composite weight for every edge (zero token).

    Signals:
      1. Explicit wikilink — 3.0 (for EXTRACTED edges)
      2. Source overlap   — Jaccard × 4.0
      3. Adamic-Adar      — Σ 1/log(deg(n)) × 1.5
      4. Type affinity    — Predefined matrix × 1.0
      5. LLM confidence   — Existing confidence for INFERRED edges (bonus multiplier)

    The weight is a SUM of non-zero signals so a wikilink (3.0) + source overlap
    (2.0) + Adamic-Adar (1.0) = 6.0. Edge attribute 'weight' is always set.
    """
    neighbor_index = _build_neighbor_index(edges)
    ...
    for edge in edges:
        ...
        # Signal 1: explicit wikilink
        s1 = 3.0 if etype == "EXTRACTED" else 0.0

        # Signal 5: existing LLM confidence (for INFERRED edges)
        s5 = edge.get("confidence", 0) if etype in ("INFERRED", "AMBIGUOUS") else 0.0

        # Signals 2-4
        s2 = _source_overlap(nodes, src, tgt)
        s3 = _adamic_adar(neighbor_index, src, tgt)
        s4 = _type_affinity(nodes, src, tgt)

        edge["weight"] = round(s1 + s2 + s3 + s4 + s5, 4)
        edge["weight_components"] = {
            "wikilink": s1,
            "source_overlap": round(s2, 4),
            "adamic_adar": round(s3, 4),
            "type_affinity": round(s4, 4),
            "llm_confidence": s5,
        }
...
# ═══════════════════════════════════════════════════════════════════════
# Graph Insights (algorithmic, zero token + optional LLM report)
# ═══════════════════════════════════════════════════════════════════════

def find_structural_insights(nodes: list[dict], edges: list[dict],
                             communities: dict[str, int]) -> dict:
    """Pure-algorithm structural graph analysis. Zero LLM tokens."""
    ...
    # Isolated / near-isolated nodes
    orphans = [nid for nid in node_ids if degree.get(nid, 0) <= 1]
    ...
    if len(degrees) >= 3:
        ...
        hubs = [nid for nid, d in degree.items() if d > mean_deg + 2 * std_deg]
    else:
        hubs = []

    # Bridge nodes (connected to 3+ communities)
    node_communities: dict[str, set[int]] = {}
    for e in edges:
        src = e.get("source", e.get("from", ""))
        tgt = e.get("target", e.get("to", ""))
        c_src = communities.get(src, -1)
        c_tgt = communities.get(tgt, -1)
        if c_src >= 0:
            node_communities.setdefault(src, set()).add(c_src)
        if c_tgt >= 0:
            node_communities.setdefault(tgt, set()).add(c_tgt)
    bridges = [nid for nid, comm_set in node_communities.items() if len(comm_set) >= 3]

    # Phantom nodes: wikilinks referencing non-existent pages
    referenced: dict[str, int] = {}
    for n in nodes:
        content = n.get("markdown", "")
        for link in extract_wikilinks(content, unique=False):
            stem = Path(link).stem.lower() if '/' in link else link.lower()
            referenced[stem] = referenced.get(stem, 0) + 1
    phantoms = [
        (name, count)
        for name, count in referenced.items()
        if count >= 2 and name not in {n["id"].split("/")[-1].lower() for n in nodes}
        and name not in {n["label"].lower() for n in nodes}
    ]
    ...
    for e in edges:
        ...
        c_src = communities.get(src, -1)
        c_tgt = communities.get(tgt, -1)
        if c_src >= 0 and c_tgt >= 0 and c_src != c_tgt:
            cross_edges.append({
                "source": src,
                "target": tgt,
    ...
    for cid, members in comm_nodes.items():
        ...
        for e in edges:
            src = e.get("source", e.get("from", ""))
            tgt = e.get("target", e.get("to", ""))
            if (src in set(members)) and (tgt in set(members)):
                comm_edges += 1
        max_possible = len(members) * (len(members) - 1) / 2
        cohesion = comm_edges / max_possible if max_possible > 0 else 0
        if cohesion < 0.15:
            sparse_communities.append({"id": cid, "size": len(members), "cohesion": round(cohesion, 4)})

    return {
        "orphan_nodes": orphans,
        "hub_nodes": hubs,
        "bridge_nodes": bridges,
        "phantom_nodes": [{"name": name, "references": count} for name, count in phantoms[:20]],
        "cross_community_edges": cross_edges[:20],
        "sparse_communities": sparse_communities,
    }


INSIGHTS_PROMPT = """基于以下知识图谱的结构分析结果，生成一份中文洞察报告。

## 图谱概况
- 节点数: {node_count}, 边数: {edge_count}
- 社区数: {community_count}
- 页面类型分布: {type_distribution}
...
1. **知识结构概览** — 图谱呈现了哪些知识聚类（根据节点名称推断社区主题），整体结构是否健康
2. **惊喜连接** — 哪些跨越不同领域/社区的连接值得关注
3. **知识缺口** — 哪些区域缺乏足够的连接，哪些重要概念没有深入
4. **改进建议** — 建议创建哪些页面，哪些区域需要补充素材

输出为 markdown，标题可读。"""


def generate_insights_report(nodes: list[dict], edges: list[dict],
                              communities: dict[str, int]) -> str:
    """Generate LLM-powered insights report from structural analysis findings."""
    structural = find_structural_insights(nodes, edges, communities)
    ...
    type_dist_str = ", ".join(f"{k}: {v}" for k, v in sorted(type_dist.items()))

    community_count = len(set(c for c in communities.values() if c >= 0))

    # Format orphan list
    orphans_str = ", ".join(structural["orphan_nodes"][:30]) if structural["orphan_nodes"] else "(无)"
    if len(structural["orphan_nodes"]) > 30:
        orphans_str += f" ... 共 {len(structural['orphan_nodes'])} 个"

    # Format hub list
    hubs_str = ", ".join(structural["hub_nodes"][:10]) if structural["hub_nodes"] else "(无)"

    # Format phantom list
    phantoms_str = "\n".join(
        f"- **{p['name']}** — 被引用 {p['references']} 次"
        for p in structural["phantom_nodes"][:15]
    ) if structural["phantom_nodes"] else "(无)"
    ...
    # Format sparse communities
    sparse_str = "\n".join(
        f"- 社区 {c['id']}: {c['size']} 节点, cohesion={c['cohesion']}"
        for c in structural["sparse_communities"]
    ) if structural["sparse_communities"] else "(无)"
    ...
    prompt = INSIGHTS_PROMPT.format(
        ...
        orphan_nodes=orphans_str,
        hub_nodes=hubs_str,
        phantom_nodes=phantoms_str,
        cross_edges=cross_str,
...
def build_graph(infer: bool = True, open_browser: bool = False, clean: bool = False,
                report: bool = False, save: bool = False, insights: bool = False):
    ...
    # Generate insights report (LLM-powered, new feature)
    if insights:
        print("  Generating insights report ...")
        insights_text = generate_insights_report(nodes, edges, communities)
        insights_path = GRAPH_DIR / "graph-insights.md"
        insights_path.write_text(insights_text, encoding="utf-8")
        print(f"  saved: graph/graph-insights.md")
        append_log(f"## [{today}] insights | Graph insights report generated\n\n{len(nodes)} nodes analyzed.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--insights", action="store_true", help="Generate LLM-powered insights report")
    args = parser.parse_args()
    build_graph(infer=not args.no_infer, open_browser=args.open, clean=args.clean,
                report=args.report, save=args.save, insights=args.insights)
